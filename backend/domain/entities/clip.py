...
A Clip represents a segment of video that could be exported as a
short-form clip. It is generated by the AI pipeline or created
manually by the user.

Business rules:
    - Clip must have valid start < end timestamps (SRS §6.5)
    - Minimum clip duration: 3 seconds (per IMPLEMENTATION_PLAN)
    - Maximum clip duration: 90 seconds (per PRD-CLIP-011)
    - Quality score range: 0-100 (SRS §6)
    - Clip states: candidate → accepted/rejected/modified (SRS F-03)
"""
...
class Clip:
    ...
    def _validate(self) -> None:
        ...
        MIN_DURATION = 3000  # 3 seconds
        if self.duration_ms < MIN_DURATION:
            raise InvalidClipRangeError(
                f"Clip duration ({self.duration_ms}ms) is below minimum ({MIN_DURATION}ms)",
                {"duration_ms": self.duration_ms, "min_duration_ms": MIN_DURATION},
            )
        MAX_DURATION = 90_000  # 90 seconds
        if self.duration_ms > MAX_DURATION:
            raise InvalidClipRangeError(
                f"Clip duration ({self.duration_ms}ms) exceeds maximum ({MAX_DURATION}ms)",
                {"duration_ms": self.duration_ms, "max_duration_ms": MAX_DURATION},
            )
        ...
        if self.hook_score is not None:
            if not (0 <= self.hook_score <= 100):
                raise DomainValidationError(
    ...
    def set_timestamps(self, start_ms: int, end_ms: int) -> None:
        """Update the clip's timestamps.

        Args:
            start_ms: New start timestamp.
            end_ms: New end timestamp.
        """
    ...
    def set_rank(self, rank: int) -> None:
        """Set the quality ranking position.

        Args:
            rank: Ranking position (1 = best, must be positive).
        """
        if rank < 1:
            from backend.domain.exceptions import DomainValidationError
    ...
    def set_scores(
        ...
        Args:
            ...
            virality: New virality score (0-100).
            hook: New hook score (0-100).
        """
    ...
    def merge_with(self, other: Clip) -> Clip:
        """Merge this clip with an overlapping clip.

        Both clips must share the same video_id. The merged clip
        extends from the earliest start to the latest end.
        """
        if self.video_id != other.video_id:
            from backend.domain.exceptions import DomainValidationError
        ...
        if not self.overlaps_with(other):
            ...
            raise DomainValidationError(
                ...
                {"clip1": f"{self.start_ms}-{self.end_ms}", "clip2": f"{other.start_ms}-{other.end_ms}"},
            )
        ...
        merged = Clip(
            ...
            quality_score=max(
                ...
                other.quality_score or 0,
            ),
            virality_score=max(
                self.virality_score or 0,
                other.virality_score or 0,
            ),
            hook_score=max(
                self.hook_score or 0,
                other.hook_score or 0,
            ),
        )
        # Combine hashtags
